...
#atas<-arch[,grep("8001-", names(arch))]
#atas$date<-arch$date
#names(atas)<-rename(atas)
#write.csv(atas, file="arch-atas.csv", row.names=FALSE)

#red<-arch[,grep("8005-", names(arch))]
#red$date<-arch$date
#names(red)<-c("o3", "wsv", "wdv", "sigt", "amt", "date")
#write.csv(red, file="arch-red.csv", row.names=FALSE)

#carr<-arch[,grep("8006-", names(arch))]
#carr$date<-arch$date
#names(carr)<-c("o3", "wsv", "wdv", "sigt", "amt", "date")
#write.csv(carr, file="arch-carr.csv", row.names=FALSE)

#morro<-arch[,grep("3001-", names(arch))]
#morro$date<-arch$date
#names(morro)<-c("no", "no2", "nox", "o3", "wsv", "wdv", "sigt", "date")
#write.csv(morro, file="arch-morro.csv", row.names=FALSE)

#grov<-arch[,grep("2001-", names(arch))]
#grov$date<-arch$date
#names(grov)<-c("wsv", "wdv", "sigt", "date")
#write.csv(grov, file="arch-grov.csv", row.names=FALSE)

#cdf<-arch[,grep("2007-", names(arch))]
#cdf$date<-arch$date
#names(cdf)<-c("wsv", "wdv", "sigt","bam10", "bam25", "date")
#write.csv(cdf, file="arch-cdf.csv", row.names=FALSE)

#mesa<-arch[,grep("2004-", names(arch))]
#mesa$date<-arch$date
#names(mesa)<-c("so2", "wsv", "wdv", "sigt","amt", "bam10", "bam25", "date")
#write.csv(mesa, file="arch-mesa.csv", row.names=FALSE)

#nrp<-arch[,grep("4002-", names(arch))]
#nrp$date<-arch$date
#nrp<-nrp[,-1]   #get rid of so2
#names(nrp)<-c("no", "no2", "nox", "o3", "wsv", "wdv", "sigt", "amt", "bam10", "date")
#write.csv(nrp, file="arch-nrp.csv", row.names=FALSE)

#rm(arch, atas, carr, cdf, grov, mesa, morro, nrp, red)

######################################
## Dec 2016: bring in Oso 2015 data ##
######################################

#arch<-read.table("oso_dump.txt", sep = "|") # load AQS data dump (AMP501)
#arch<-arch[,-c(1:4, 14:28)]                # remove unwanted columns
#names(arch)<-c("site", "param", "poc", "dur", "unit", "method", "date", "time", "value")
#arch<-arch[-which(arch$param==81102),]     # get rid of pm10std
#arch<-arch[,-4]                            # get rid of duration column, now that it's all hourly

#create site id
#arch$siteid<-paste(arch$site, arch$param, arch$poc, arch$method, sep="-")
#unique(arch$siteid) #check
...
#long to short...
#library(reshape2)
#arch<-dcast(arch, date~siteid, value.var="value")
#summary(arch)

##write file
#oso <- arch
#names(oso)<-c("date", "wsv", "wdv", "sigt", "bam10")
#write.csv(oso, file="arch-oso.csv", row.names=FALSE)
